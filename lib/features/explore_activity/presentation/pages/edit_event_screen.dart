...
class _EditEventScreenState extends State<EditEventScreen> {
  ...
  void _populateFields() {
    final details = eventController.eventDetails.value;
  ...
  void _updateHandler() async {
    ...
    final event = eventController.eventDetails.value?.eventModel;
    final eventDetails = eventController.eventDetails.value;
    if (event == null) return;

    final request = EventUpdateRequest(
      eventId: eventId,
      title: titleTEController.text != event.title
          ? titleTEController.text
          : null,
      details: detailsTEController.text != event.description
          ? detailsTEController.text
          : null,
      location: locationTEController.text != event.location
          ? locationTEController.text
          : null,
      url: mapUrlTEController.text != eventDetails?.mapUrl
      ...
      maxParticipants:
          ...
          : null,
      isPublic: isPublic.value != event.isPublic ? isPublic.value : null,
    ...
    if (success) {
      ...
      SnackbarService.error(
        eventUpdateController.errorMessage.value ?? "Update failed",
      );
    }
  }
  ...
  void showCalendar() async {
    DateTime? pickedDate = await showDatePicker(
      ...
      initialDate: selectedDate.value ?? DateTime.now(), // Uses populated date
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Obx(() {
        final controller = Get.find<BusinessEventDetailsController>();
        ...
        if (controller.errorMessage.value != null) {
          return ErrorStateWidget(
            ...
            onRetry: () => eventController.fetchEventDetails(eventId),
          );
        }
        ...
        final details = controller.eventDetails.value;
        final event = details?.eventModel;
  ...
  Widget _buildBottomButtons(ColorScheme colorScheme) {
    return Row(
      children: [
        ...
        Expanded(
          child: Obx(() {
            final controller = Get.find<BusinessEventUpdateController>();
            formTick.value;
            return CustomButton(
  ...
  void _showBottomSheet() {
    ...
    final localTitleController = TextEditingController(
      text: titleTEController.text,
    );
    final localDetailsController = TextEditingController(
      text: detailsTEController.text,
    );
                ...
                children: [
                  ...
                  CustomButton(
                    ...
                    onPressed: () {
                      ...
                      detailsTEController.text = localDetailsController.text
                          .trim();
                      formTick.value++;
