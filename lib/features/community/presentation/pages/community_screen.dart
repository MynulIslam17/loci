...
class _CommunityScreenState extends State<CommunityScreen>
  ...
  Future<void> _onPostQuestion(
    ...
    if (success) {
      ...
      SnackbarService.error(
        questionController.errorMessage.value ?? "Something went wrong",
      );
    }
  }
  ...
  Future<void> _onMentionSubmit(
    ...
    if (updated != null) {
      ...
      final message = postPollController.errorMessage.value ?? 'Failed to add option';
      // Duplicate option isn't a real error — silently ignore it (no snackbar).
  ...
  void _onVote(String announcementId, String optionId) async {
    ...
    if (success) {
      ...
      SnackbarService.error(voteController.errorMessage.value ?? "Vote failed");
    }
  }
    ...
    return Scaffold(
      ...
      body: NestedScrollView(
        headerSliverBuilder: (context, _) => [
          ...
          _CommunityTabBar(controller: tabController, colorScheme: colorScheme),
        ],
        body: Padding(
...
class _FeedShimmer extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Column(children: List.generate(3, (_) => const _PostCardSkeleton()));
  }
}
